...
namespace XD2206DataImporter
{
    public class TransItem
    {
        ...
        public ToDevice ToDevice
        {
            get
            {
                if (_toDevice == null)
                {
                    _toDevice = new ToDevice(_writeDelegate);
                }
                return _toDevice;
            }
            set
    ...
    public class ToDevice : DeviceBase
    {
        ...
        private void WriteDataRow(DataRow row)
        {
            ...
            float beforeDepth = Convert.ToSingle(SafeFloat(row["BeforeDepth"]));
            ...
            // convert mm to cm
            // 
            beforeDepth = MMToCM(beforeDepth);

            Hashtable hs = new Hashtable();

            hs[DataEnum.DeviceID] = this.ID;
            hs[DataEnum.DT] = dt;
            hs[DataEnum.BeforeDepth] = beforeDepth;
            hs[DataEnum.Discharge] = discharge;
            hs[DataEnum.TotalWaterVolum] = totalWaterVolum;
            hs[DataEnum.Elvation] = this.Elvation;
